...
# -----------------------------------------------------------------------------
# Messaging models & quota config moved to routes/messaging.py
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Articles — moved to routes/articles.py (see create_articles_router).
# -----------------------------------------------------------------------------
...
# SubscribeRequest / ReminderRequest models moved to routes/public.py.
...
# -----------------------------------------------------------------------------
# Paid messaging + inbox/sent + quota + push diagnostics moved to
# routes/messaging.py — see create_messaging_router() and record_inbox_rows().
# -----------------------------------------------------------------------------
...
# Inbox row writer for reminders imported as `record_inbox_rows` from
# routes/messaging.py (declared alongside the messaging endpoints).
...
# -----------------------------------------------------------------------------
# Articles module extracted to routes/articles.py — see create_articles_router
# and init_articles() which are wired into api_router / on_startup respectively.
# -----------------------------------------------------------------------------


# -----------------------------------------------------------------------------
# Bot prerender + iCal feed + newsletter + per-event reminder moved to
# routes/public.py — see create_public_router().
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Merchant Cards (admin management + activation requests) and Event Organizer
# request/contact flow moved to routes/merchants.py.
# See create_merchants_router() plus scheduler jobs merchant_card_expiry_sweep
# and organizer_sync_job (both imported at the top of this file).
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Startup / shutdown — extracted to bootstrap.py.
# The scheduler handle survives across the two events so shutdown can stop it.
# -----------------------------------------------------------------------------
scheduler = None
...
# Sitemap moved to routes/public.py — see create_public_router().
...
api_router.include_router(
    create_messaging_router(db, get_current_user, get_admin_or_moderator)
)
api_router.include_router(
